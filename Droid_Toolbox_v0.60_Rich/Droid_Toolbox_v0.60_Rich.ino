/* Droid Toolbox v0.60A : ruthsarian@gmail.com
 *  
 *  A = Aurebesh. Added by Phiric
 * 
 ...
 *   
 * Mods by Rich Phillips
 *  4/18/2023 - Added Aurebesh description of Location Beacons.
 */
...
#include "Aurebesh8pt7b.h"                        // copied into sketch folder
...
#define MSG_VERSION                         "v0.60A"                // the version displayed on the splash screen at the lower right, A=Aurebesh
...
void display_beacon_control() {
  ...
  y += tft.fontHeight() + (BEACON_CONTROL_TEXT_PADDING);
  // for Aurebesh
  tft.setFreeFont(&Aurebesh8pt7b);  // Select Aurebesh font
  tft.setTextSize(1);
  tft.setTextColor(DROID_REPORT_RSSI_COLOR);    // purple
  tft.drawString(msg, tft.getViewportWidth()/2, y);
  snprintf(msg, MSG_LEN_MAX, "%s", msg_locations[beacon.setting[0]]);   // snprintf() is for serial.print
  y -= tft.fontHeight() + (BEACON_CONTROL_TEXT_PADDING);
  tft.setTextFont(NULL);     // Select the orginal font by using NULL
  tft.setTextSize(BEACON_CONTROL_TEXT_SIZE);   
